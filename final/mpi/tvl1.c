...
int main( int argc, char *argv[]){
  ...
  //do tvl1 mpi version
  {
    ...
    int iteration = ITER;
    float norm;
    float dx_v, dy_v, dx_inner_v, dy_inner_v, nt, x1, x, imagecopy;
    ...
    for(iter = 0; iter < iteration; iter++){
      ...
      for(i = 0; i < partialHeight; i++){
        for(j = 0; j < width; j++){
          ...
          x = imagecopy - nt * fTau;
          x1 = x + max(min(partialData[i][j] - x, shrink), -shrink);
  ...
  if(rank == 0){ 
    ...
    printf("Time elapsed for TVL1 MPI solver is %f seconds.\n", elapsed);
  }

  //save to file
  if(rank == 0){
    FILE *outputFile = fopen("../tvl1ResultFile.txt", "w+");
