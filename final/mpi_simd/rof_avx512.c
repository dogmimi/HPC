...
int main( int argc, char *argv[]){
  ...
  //do rof mpi version
  {
    ...
    float *fTempMask = (float*) valloc(sizeof(float) * 16);
    float fTheta = 1.0;
    int iteration = ITER;
    float norm;
    ...
    //lt = clambda * tau
    __m512 xmm_sigma_mask = _mm512_set1_ps(fSigma);
    __m512 xmm_zero_mask = _mm512_set1_ps(0.0);
    __m512 xmm_ones_mask = _mm512_set1_ps(1.0);
    __m512 xmm_tau_mask = _mm512_set1_ps(fTau);
    __m512 xmm_lt_mask = _mm512_set1_ps(lt);
    __m512 xmm_lt1_mask = _mm512_set1_ps(lt + 1.0);
    __m512 xmm_theta_mask = _mm512_set1_ps(fTheta);
    __m512 xmm_norm1;
    __m512 xmm_norm2;
    for(iter = 0; iter < iteration; iter++){
      ...
      for(i = 0; i < partialHeight; i++){
        ...
        __m512 xmm_dx, xmm_dy, xmm_dx_inner_v, xmm_dy_inner_v, xmm_shift_x_imagecopy, xmm_shift_y_imagecopy, xmm_imagecopy;
        for(j = 0; j < width / 16 - 1; j++){//j stands for block index, block size is 16
          xmm_shift_x_imagecopy = _mm512_loadu_ps(shift_x_imagecopy_ptr);
          xmm_imagecopy = _mm512_loadu_ps(imagecopy_ptr);
          xmm_dx_inner_v = _mm512_sub_ps(xmm_shift_x_imagecopy, xmm_imagecopy);
          xmm_dx_inner_v = _mm512_mul_ps(xmm_dx_inner_v, xmm_sigma_mask);

          if(rank == (num_tasks - 1) && (i == (partialHeight - 1))){
            xmm_dy_inner_v = xmm_zero_mask;
          }else{
            xmm_shift_y_imagecopy = _mm512_loadu_ps(shift_y_imagecopy_ptr);
            xmm_dy_inner_v = _mm512_sub_ps(xmm_shift_y_imagecopy, xmm_imagecopy);
            xmm_dy_inner_v = _mm512_mul_ps(xmm_dy_inner_v, xmm_sigma_mask);
          }

          xmm_dx = _mm512_loadu_ps(dx_ptr);
          xmm_dy = _mm512_loadu_ps(dy_ptr);
          /*if(i == 0 && j == 0){
            printf("dx_v: %f\n", dx_ptr[0]);
            printf("dy_v: %f\n", dy_ptr[0]);
             float test[4];
          ...
          xmm_dx_inner_v = _mm512_add_ps(xmm_dx, xmm_dx_inner_v);
          xmm_dy_inner_v = _mm512_add_ps(xmm_dy, xmm_dy_inner_v);

          xmm_norm1 = _mm512_mul_ps(xmm_dx_inner_v, xmm_dx_inner_v);
          xmm_norm2 = _mm512_mul_ps(xmm_dy_inner_v, xmm_dy_inner_v);
          xmm_norm1 = _mm512_add_ps(xmm_norm1, xmm_norm2);
          xmm_norm1 = _mm512_sqrt_ps(xmm_norm1);
          xmm_norm1 = _mm512_max_ps(xmm_norm1, xmm_ones_mask);
          xmm_dx_inner_v = _mm512_div_ps(xmm_dx_inner_v, xmm_norm1);
          xmm_dy_inner_v = _mm512_div_ps(xmm_dy_inner_v, xmm_norm1);

          _mm512_store_ps(&partialDx[i][j * 16], xmm_dx_inner_v);
          _mm512_store_ps(&partialDy[i + 1][j * 16], xmm_dy_inner_v);

          shift_x_imagecopy_ptr += 16;//shift 16 pixels
          shift_y_imagecopy_ptr += 16;//shift 16 pixels
          imagecopy_ptr += 16;//shift 16 pixels
          dx_ptr += 16;//shift 16 pixels
          dy_ptr += 16;//shift 16 pixels
        }
        xmm_imagecopy = _mm512_loadu_ps(imagecopy_ptr);
        //printf("!!\n");
        int k = 0;
        int lastBlock = (width / 16 - 1) * 16;
        for(j = lastBlock; j < width - 1; j++){//here j stands for width index, only deal 15 pixels!
          fTempMask[k] = (partialImageCopy[i][j + 1] - imagecopy_ptr[k]) * fSigma;
          k++;
        }
        fTempMask[15] = 0.0;
        xmm_dx_inner_v = _mm512_load_ps(fTempMask);
        if(rank == (num_tasks - 1) && (i == (partialHeight - 1))){
          xmm_dy_inner_v = xmm_zero_mask;
        }else{
          xmm_shift_y_imagecopy = _mm512_loadu_ps(shift_y_imagecopy_ptr);
        ...
        xmm_dx = _mm512_load_ps(dx_ptr);
        xmm_dy = _mm512_load_ps(dy_ptr);
        xmm_dx_inner_v = _mm512_add_ps(xmm_dx, xmm_dx_inner_v);
        xmm_dy_inner_v = _mm512_add_ps(xmm_dy, xmm_dy_inner_v);
        ...
        _mm512_store_ps(&partialDx[i][lastBlock], xmm_dx_inner_v);
        _mm512_store_ps(&partialDy[i + 1][lastBlock], xmm_dy_inner_v);
      ...
      for(i = 0; i < partialHeight; i++){
        __m512 xmm_temp1, xmm_temp2, xmm_x1, xmm_imagecopy;
        float* dx_ptr = partialDx[i];
        float* dy_ptr = partialDy[i + 1];
        float* shift_dx_ptr = &(partialDx[i][15]);
        float* shift_dy_ptr = partialDy[i];
        float* imagecopy_ptr = partialImageCopy[i];
        float* data_ptr = partialData[i];

        for(j = 0; j < width / 16; j++){
          __m512 xmm_nt = xmm_zero_mask;
          if(j == 0){//first line
            xmm_temp1 = _mm512_loadu_ps(dx_ptr);
            xmm_temp2 = _mm512_setr_ps(0.0, partialDx[i][0], partialDx[i][1], partialDx[i][2], partialDx[i][3], partialDx[i][4], partialDx[i][5], partialDx[i][6], partialDx[i][7],partialDx[i][8],partialDx[i][9],partialDx[i][10],partialDx[i][11],partialDx[i][12],partialDx[i][13],partialDx[i][14]);
            //if(i == 0){
            //  printf("dx: %f, pDx: %f\n", dx_ptr[1], partialDx[i][0]);
            //}
          }else if(j == (width / 16 - 1)){//last line
            xmm_temp1 = _mm512_setr_ps(dx_ptr[0], dx_ptr[1], dx_ptr[2], dx_ptr[3], dx_ptr[4], dx_ptr[5], dx_ptr[6], dx_ptr[7], dx_ptr[8], dx_ptr[9], dx_ptr[10], dx_ptr[11], dx_ptr[12], dx_ptr[13], dx_ptr[14], 0.0);
            xmm_temp2 = _mm512_loadu_ps(shift_dx_ptr);
          }else{
            xmm_temp1 = _mm512_loadu_ps(dx_ptr);
            xmm_temp2 = _mm512_loadu_ps(shift_dx_ptr);
            shift_dx_ptr += 16;
          ...
          xmm_nt = _mm512_sub_ps(xmm_nt, xmm_temp1);
          xmm_nt = _mm512_add_ps(xmm_nt, xmm_temp2);
          xmm_temp1 = xmm_zero_mask;
          xmm_temp2 = xmm_zero_mask;

          if((rank == 0) && (i == 0)){//first line
            xmm_temp1 = _mm512_loadu_ps(dy_ptr);
          //last line
          }else if((rank == (num_tasks - 1)) && (i == (partialHeight - 1))){
            xmm_temp2 = _mm512_loadu_ps(shift_dy_ptr);
          //others
          }else{
            xmm_temp1 = _mm512_loadu_ps(dy_ptr);
            xmm_temp2 = _mm512_loadu_ps(shift_dy_ptr);
          ...
          xmm_temp1 = _mm512_loadu_ps(imagecopy_ptr);
          xmm_temp2 = _mm512_loadu_ps(data_ptr);
          xmm_nt = _mm512_mul_ps(xmm_nt, xmm_tau_mask);
          xmm_nt = _mm512_sub_ps(xmm_temp1, xmm_nt);
          xmm_temp2 = _mm512_mul_ps(xmm_lt_mask, xmm_temp2);
          xmm_nt = _mm512_add_ps(xmm_nt, xmm_temp2);
          xmm_nt = _mm512_div_ps(xmm_nt, xmm_lt1_mask);

          xmm_temp2 = _mm512_sub_ps(xmm_nt, xmm_temp1);
          xmm_temp2 = _mm512_mul_ps(xmm_theta_mask, xmm_temp2);
          xmm_nt = _mm512_add_ps(xmm_nt, xmm_temp2);

          _mm512_store_ps(&partialImageCopy[i][j * 16], xmm_nt);
  ...
  if(rank == 0){
    ...
    printf("Time elapsed for ROF MPI+AVX512 solver is %f seconds.\n", elapsed);
  }

  //save to file
  if(rank == 0){
    FILE *outputFile = fopen("../rofAvx512ResultFile.txt", "w+");
