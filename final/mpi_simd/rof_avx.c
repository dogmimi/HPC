...
int main( int argc, char *argv[]){
    ...
    for(iter = 0; iter < iteration; iter++){
      ...
      for(i = 0; i < partialHeight; i++){
        ...
        for(j = 0; j < width / 8; j++){
          ...
          xmm_temp2 = _mm256_mul_ps(xmm_lt_mask, xmm_temp2);
          xmm_nt = _mm256_add_ps(xmm_nt, xmm_temp2);
          xmm_nt = _mm256_div_ps(xmm_nt, xmm_lt1_mask);

          xmm_temp2 = _mm256_sub_ps(xmm_nt, xmm_temp1);
          xmm_temp2 = _mm256_mul_ps(xmm_theta_mask, xmm_temp2);
          xmm_nt = _mm256_add_ps(xmm_nt, xmm_temp2); 

          _mm256_store_ps(&partialImageCopy[i][j * 8], xmm_nt);
        ...
        for(j = 0; j < width; j++){
          ...
          if((rank == 0) && (i == 0)){
            nt -= dy_v;
          //need to deal with last line line
          }else if((rank == (num_tasks - 1)) && (i == (partialHeight - 1))){
            nt += partialDy[i][j];
          //need to deal with other lines
          }else{
            nt -= dy_v;
            nt += partialDy[i][j];
          }
  ...
  //save to file
  if(rank == 0){
    FILE *outputFile = fopen("../rofAvxResultFile.txt", "w+");
