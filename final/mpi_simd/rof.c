...
int main( int argc, char *argv[]){
    ...
    for(iter = 0; iter < iteration; iter++){
      ...
      for(i = 0; i < partialHeight; i++){
        ...
        for(j = lastBlock; j < width - 1; j++){//here j stands for width index, only deal 3 pixels!
          ...
          //fTempMask[k] = (partialImageCopy[i][j + 1] - partialImageCopy[i][j]) * fSigma;
          k++;
        }
        fTempMask[3] = 0.0;
        ...
        if(i == 0){
          int k;
          for(k = 0; k < 8; k++){
            printf("%f ", partialDx[0][k]);
          }
          printf("\n");
          for(k = 0; k < 8; k++){
            printf("%f ", partialDy[0][k]);
          }
          printf("\n");
        }*/
      ...
      for(i = 0; i < partialHeight; i++){
        ...
        for(j = 0; j < width / 4; j++){
          ...
          xmm_temp2 = _mm_mul_ps(xmm_lt_mask, xmm_temp2);
          xmm_nt = _mm_add_ps(xmm_nt, xmm_temp2);
          xmm_nt = _mm_div_ps(xmm_nt, xmm_lt1_mask);

          xmm_temp2 = _mm_sub_ps(xmm_nt, xmm_temp1);
          xmm_temp2 = _mm_mul_ps(xmm_theta_mask, xmm_temp2);
          xmm_nt = _mm_add_ps(xmm_nt, xmm_temp2); 

          _mm_store_ps(&partialImageCopy[i][j * 4], xmm_nt);
  ...
  //save to file
  if(rank == 0){
    FILE *outputFile = fopen("../rofSSEResultFile.txt", "w+");
