...
int main( int argc, char *argv[]){
  ...
  //do rof mpi version
  {
    ...
    float *fTempMask = (float*) valloc(sizeof(float) * 8);
    float fTheta = 1.0;
    int iteration = ITER;
    float norm;
    ...
    //lt = clambda * tau
    __m256 xmm_sigma_mask = _mm256_set1_ps(fSigma);
    __m256 xmm_zero_mask = _mm256_set1_ps(0.0);
    __m256 xmm_ones_mask = _mm256_set1_ps(1.0);
    __m256 xmm_tau_mask = _mm256_set1_ps(fTau);
    __m256 xmm_lt_mask = _mm256_set1_ps(lt);
    __m256 xmm_lt1_mask = _mm256_set1_ps(lt + 1.0);
    __m256 xmm_theta_mask = _mm256_set1_ps(fTheta);
    __m256 xmm_shrink1_mask = _mm256_set1_ps(shrink);
    __m256 xmm_shrink2_mask = _mm256_set1_ps(-shrink);
    __m256 xmm_norm1;
    __m256 xmm_norm2;
    for(iter = 0; iter < iteration; iter++){
      ...
      for(i = 0; i < partialHeight; i++){
        ...
        __m256 xmm_dx, xmm_dy, xmm_dx_inner_v, xmm_dy_inner_v, xmm_shift_x_imagecopy, xmm_shift_y_imagecopy, xmm_imagecopy;
        for(j = 0; j < width / 8 - 1; j++){//j stands for block index, block size is 8
          xmm_shift_x_imagecopy = _mm256_loadu_ps(shift_x_imagecopy_ptr);
          xmm_imagecopy = _mm256_loadu_ps(imagecopy_ptr);
          xmm_dx_inner_v = _mm256_sub_ps(xmm_shift_x_imagecopy, xmm_imagecopy);
          xmm_dx_inner_v = _mm256_mul_ps(xmm_dx_inner_v, xmm_sigma_mask);

          if(rank == (num_tasks - 1) && (i == (partialHeight - 1))){
            xmm_dy_inner_v = xmm_zero_mask;
          }else{
            xmm_shift_y_imagecopy = _mm256_loadu_ps(shift_y_imagecopy_ptr);
            xmm_dy_inner_v = _mm256_sub_ps(xmm_shift_y_imagecopy, xmm_imagecopy);
            xmm_dy_inner_v = _mm256_mul_ps(xmm_dy_inner_v, xmm_sigma_mask);
          }

          xmm_dx = _mm256_loadu_ps(dx_ptr);
          xmm_dy = _mm256_loadu_ps(dy_ptr);
          /*if(i == 0 && j == 0){
            printf("dx_v: %f\n", dx_ptr[0]);
            printf("dy_v: %f\n", dy_ptr[0]);
             float test[4];
            _mm256_store_ps(&test, xmm_dx);
            printf("dx_v: %f\n", test[0]);
            _mm256_store_ps(&test, xmm_dy);
            printf("dy_v: %f\n", test[0]);
          }*/
          xmm_dx_inner_v = _mm256_add_ps(xmm_dx, xmm_dx_inner_v);
          xmm_dy_inner_v = _mm256_add_ps(xmm_dy, xmm_dy_inner_v);
          
          xmm_norm1 = _mm256_mul_ps(xmm_dx_inner_v, xmm_dx_inner_v);
          xmm_norm2 = _mm256_mul_ps(xmm_dy_inner_v, xmm_dy_inner_v);
          xmm_norm1 = _mm256_add_ps(xmm_norm1, xmm_norm2);
          xmm_norm1 = _mm256_sqrt_ps(xmm_norm1);
          xmm_norm1 = _mm256_max_ps(xmm_norm1, xmm_ones_mask);
          xmm_dx_inner_v = _mm256_div_ps(xmm_dx_inner_v, xmm_norm1);
          xmm_dy_inner_v = _mm256_div_ps(xmm_dy_inner_v, xmm_norm1);
        
          _mm256_store_ps(&partialDx[i][j * 8], xmm_dx_inner_v);
          _mm256_store_ps(&partialDy[i + 1][j * 8], xmm_dy_inner_v);

          shift_x_imagecopy_ptr += 8;//shift 8 pixels
          shift_y_imagecopy_ptr += 8;//shift 8 pixels
          imagecopy_ptr += 8;//shift 8 pixels
          dx_ptr += 8;//shift 8 pixels
          dy_ptr += 8;//shift 8 pixels
        }
        xmm_imagecopy = _mm256_loadu_ps(imagecopy_ptr);
        //printf("!!\n");
        int k = 0;
        int lastBlock = (width / 8 - 1) * 8;
        for(j = lastBlock; j < width - 1; j++){//here j stands for width index, only deal 7 pixels!
          fTempMask[k] = (partialImageCopy[i][j + 1] - imagecopy_ptr[k]) * fSigma;
          k++;
        }
        fTempMask[7] = 0.0;
        xmm_dx_inner_v = _mm256_load_ps(fTempMask);
        if(rank == (num_tasks - 1) && (i == (partialHeight - 1))){
          xmm_dy_inner_v = xmm_zero_mask;
        }else{
          xmm_shift_y_imagecopy = _mm256_loadu_ps(shift_y_imagecopy_ptr);
        ...
        xmm_dx = _mm256_load_ps(dx_ptr);
        xmm_dy = _mm256_load_ps(dy_ptr);
        xmm_dx_inner_v = _mm256_add_ps(xmm_dx, xmm_dx_inner_v);
        xmm_dy_inner_v = _mm256_add_ps(xmm_dy, xmm_dy_inner_v);
        ...
        _mm256_store_ps(&partialDx[i][lastBlock], xmm_dx_inner_v);
        _mm256_store_ps(&partialDy[i + 1][lastBlock], xmm_dy_inner_v);
        //printf("e-i: %d, j: %d\n", i, j);
      ...
      for(i = 0; i < partialHeight; i++){
        __m256 xmm_temp1, xmm_temp2, xmm_x1, xmm_imagecopy;
        float* dx_ptr = partialDx[i];
        float* dy_ptr = partialDy[i + 1];
        float* shift_dx_ptr = &(partialDx[i][7]);
        float* shift_dy_ptr = partialDy[i];
        float* imagecopy_ptr = partialImageCopy[i];
        float* data_ptr = partialData[i];
        
        for(j = 0; j < width / 8; j++){
          __m256 xmm_nt = xmm_zero_mask;
          if(j == 0){//first line
            xmm_temp1 = _mm256_loadu_ps(dx_ptr); 
            xmm_temp2 = _mm256_setr_ps(0.0, partialDx[i][0], partialDx[i][1], partialDx[i][2], partialDx[i][3], partialDx[i][4], partialDx[i][5], partialDx[i][6]);
            //if(i == 0){
            //  printf("dx: %f, pDx: %f\n", dx_ptr[1], partialDx[i][0]);
            //}
          }else if(j == (width / 8 - 1)){//last line
            xmm_temp1 = _mm256_setr_ps(dx_ptr[0], dx_ptr[1], dx_ptr[2], dx_ptr[3], dx_ptr[4], dx_ptr[5], dx_ptr[6], 0.0);
            xmm_temp2 = _mm256_loadu_ps(shift_dx_ptr); 
          }else{
            xmm_temp1 = _mm256_loadu_ps(dx_ptr); 
            xmm_temp2 = _mm256_loadu_ps(shift_dx_ptr); 
            shift_dx_ptr += 8;
          ...
          xmm_nt = _mm256_sub_ps(xmm_nt, xmm_temp1);
          xmm_nt = _mm256_add_ps(xmm_nt, xmm_temp2);
          /*if(i == 0 && j == 0){
             float test[4];
            _mm256_store_ps(&test, xmm_temp1);
            printf("tmp10: %f\n", test[0]);
            printf("tmp11: %f\n", test[1]);
            printf("tmp12: %f\n", test[2]);
            printf("tmp13: %f\n", test[3]);
            _mm256_store_ps(&test, xmm_temp2);
            printf("tmp20: %f\n", test[0]);
            printf("tmp21: %f\n", test[1]);
            printf("tmp22: %f\n", test[2]);
            printf("tmp23: %f\n", test[3]);
            _mm256_store_ps(&test, xmm_nt);
            printf("nt0: %f\n", test[0]);
            printf("nt1: %f\n", test[1]);
            printf("nt2: %f\n", test[2]);
            printf("nt3: %f\n", test[3]);
          ...
          if((rank == 0) && (i == 0)){//first line
            xmm_temp1 = _mm256_loadu_ps(dy_ptr);
          //last line
          }else if((rank == (num_tasks - 1)) && (i == (partialHeight - 1))){
            xmm_temp2 = _mm256_loadu_ps(shift_dy_ptr);
          //others
          }else{
            xmm_temp1 = _mm256_loadu_ps(dy_ptr);
            xmm_temp2 = _mm256_loadu_ps(shift_dy_ptr);
          ...
          xmm_temp1 = _mm256_loadu_ps(imagecopy_ptr);
          xmm_temp2 = _mm256_loadu_ps(data_ptr);
          xmm_nt = _mm256_mul_ps(xmm_nt, xmm_tau_mask);
          xmm_nt = _mm256_sub_ps(xmm_temp1, xmm_nt);
          xmm_temp2 = _mm256_sub_ps(xmm_temp2, xmm_nt);
          xmm_temp2 = _mm256_min_ps(xmm_temp2, xmm_shrink1_mask);
          xmm_temp2 = _mm256_max_ps(xmm_temp2, xmm_shrink2_mask);
          xmm_temp2 = _mm256_add_ps(xmm_temp2, xmm_nt);

          xmm_temp1 = _mm256_sub_ps(xmm_temp2, xmm_temp1);
          xmm_temp1 = _mm256_mul_ps(xmm_temp1, xmm_theta_mask);
          xmm_temp2 = _mm256_add_ps(xmm_temp2, xmm_temp1); 

          _mm256_store_ps(&partialImageCopy[i][j * 8], xmm_temp2);
  ...
  if(rank == 0){ 
    ...
    printf("Time elapsed for ROF MPI+AVX solver is %f seconds.\n", elapsed);
  }

  //save to file
  if(rank == 0){
    FILE *outputFile = fopen("../tvl1AvxResultFile.txt", "w+");
